...
# High School Clock`
  ...
  main
    ...
    .define('size', ['mutable size'], (_) => _.generator)
  main
    .variable(observer('slider'))
    .define('slider', ['html', 'mutable size'], function (html, $0) {
  ...
  main
    ...
    .define('clock', ['size', 'd3', 'invalidation'], function (
      size,
      d3,
      invalidation
    ) {
      ...
      const hourScale = d3.scaleLinear().range([0, 360]).domain([0, 12])
      const minSecScale = d3.scaleLinear().range([0, 360]).domain([0, 60])
      ...
      function draw() {
        ...
        face
          ...
          .attr(
            'x',
            (d) => labelRadius * Math.sin((hourScale(d) * Math.PI) / 180)
          )
          .attr(
            'y',
            (d) =>
              -labelRadius * Math.cos((hourScale(d) * Math.PI) / 180) +
              labelFontSize * 0.375
          )
